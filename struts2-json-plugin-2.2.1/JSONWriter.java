...
public class JSONWriter {
    ...
    public String write(Object object, Collection<Pattern> excludeProperties,
        ...
        this.value(object, null);		//由此可见同样此方法没有进行进行实例化的实际操作，而是进行一些赋值操作，而将序列化的工作交给了value
    ...
    protected void value(Object object, Method method) throws JSONException {
        ...
        this.process(object, method);		//再次进入process方法
    }

    /**
     * Serialize object into json
     */
	 /*process方法判断是哪种数据类型，由此可见Struts-json-plungin支持哪些数据类型*/
    ...
    protected void map(Map map, Method method) throws JSONException {
        ...
        while (it.hasNext()) {
            ...
            if (key == null) {		//如果key不是String类型，将发出警告
                LOG.error("Cannot build expression for null key in #0", exprStack);
                continue;
            }
			//当前OGNL表达式，http://baike.baidu.com/link?url=KbqkAFXLzb9rN9pGYbIgc5QNIegm1geVi_iKos9FyA3aZ842LHf0UYrguWPv4imKhbC6jxISxF1cU_qrRHLW5_
            String expr = null;			
            if (this.buildExpr) {
                expr = this.expandExpr(key.toString());		//获取完整的OGNL表达式 
                if (this.shouldExcludeProperty(expr)) {		//是否是被排除的属性
                    continue;
                }
                expr = this.setExprStack(expr);		//如果不被排除，则将当前属性名压入表达式栈(其实就是一个String，而非传统意义上的栈，此处是模拟，非常精巧的算法) 
            }
            if (hasData) {
                this.add(',');	//如果还有数据，则以","风格，这是JSON的语法格式 
            }
            hasData = true;
            if (!warnedNonString && !(key instanceof String)) {	////如果key不是String类型，将发出警告，且只警告一次
            ...
            this.add(":");				//JSON语法格式
            this.value(entry.getValue(), method);	//递归抽取数据
            if (this.buildExpr) {	//下一层的数据递归完成后，恢复表达式栈值为当前层的属性名
                this.setExprStack(expr);	
            }
        }
